...
describe CategoriesController do
  before :each do
    ...
    Category.stub(:find_by_id).and_return(@hall)
  end
  ...
  describe 'access the categories index' do
    ...
    it 'should retrieve the tips from the category' do
      Category.should_receive(:get_tips)
      get :show, :id => @category.id
    end
  end
  
  describe 'create a category' do
    post 'create', {'category' =>  {'title' => 'unplug', 'content' => 'unplug your stuff man'}}
    response.should redirect_to(:action => 'show')
  end
  
  describe 'update a category' do
    post 'update', {'category_id' =>  @category.id, 'name' => 'cool sauce', 'content' => 'have lots of sauce'}
    response.should redirect_to(:action => 'show')
  end
